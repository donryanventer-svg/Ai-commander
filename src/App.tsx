...
export default function App() {
  ...
  const handleSendMessage = async (text: string, attachments: Attachment[] = []): Promise<string> => {
    if (!activeThreadId) {
      handleNewThread();
      return '';
    }
    ...
    try {
      const activePrompt = currentModel.systemPrompt || params.systemPrompt;
  ...
  return (
    ...
    >
      ...
      {toast && (
        <div
          id="forge-toast"
          className="blueprint fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#0c0c0c] border border-[#c5a47e]/70 text-[#f3f3f3] text-xs px-4 py-2.5 rounded shadow-2xl flex items-center gap-2.5 animate-in fade-in slide-in-from-bottom-2 duration-150 font-mono tracking-wide"
        >
          <i className="corner tl" />
          <i className="corner tr" />
          <i className="corner bl" />
          <i className="corner br" />
          <span className="w-1.5 h-1.5 rounded-full bg-[#c5a47e] animate-ping" />
