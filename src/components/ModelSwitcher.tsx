...
export const ModelSwitcher: React.FC<ModelSwitcherProps> = ({
  ...
  const renderIcon = (model: Model) => {
    ...
    if (model.isCustom) {
      return <Sparkles className="w-3.5 h-3.5 text-[#c5a47e] flex-none" />;
    }
    if (model.local) {
      return <Cpu className="w-3.5 h-3.5 text-[#c5a47e] flex-none" />;
    }
    return <Cloud className="w-3.5 h-3.5 text-[#c5a47e] flex-none" />;
  };
      ...
      {open && (
        ...
        >
          ...
          {customModels.length > 0 && (
            <div className="mb-3">
              <div className="text-[10px] font-semibold tracking-[0.16em] text-[#c5a47e] uppercase px-2 py-1 select-none flex items-center justify-between font-mono">
                ...
                {customModels.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-2 rounded flex items-start justify-between gap-2 transition-colors cursor-pointer ${
                        isSelected
                          ? 'bg-[#c5a47e]/15 border border-[#c5a47e]/40'
                          : 'hover:bg-[#141414] border border-transparent'
                      }`}
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold text-[#f3f3f3]">{m.name}</span>
                          <span className="text-[9px] px-1.5 py-0.2 bg-[#c5a47e]/20 text-[#c5a47e] rounded font-mono">
                            Tuned
                          </span>
                        </div>
                      ...
                      {isSelected && <Check className="w-4 h-4 text-[#c5a47e] flex-none mt-0.5" />}
                    </button>
                  );
                })}
              </div>
          ...
          <div className="pt-2 border-t border-[#1c1c1c] flex items-center gap-2 mt-2">
            <button
              ...
              className="flex-1 h-8 flex items-center justify-center gap-1.5 bg-[#1a1a1a] hover:bg-[#222222] text-[#c5a47e] text-xs font-medium rounded-sm transition-colors cursor-pointer border border-[#c5a47e]/25"
            >
